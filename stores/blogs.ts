...
import type {Blogs, BlogFormState, UpdateBlog } from '~/type';
...
export const useBlogStore = defineStore('blogs', () => {
  ...
  const fetchBlogs = async () => {
    try {
      const response = await axios.get<ApiResponse<Blogs[]>>('http://localhost:3030/v1/blogs');
  ...
  const createBlog = async (data: BlogFormState) => {
    try {
      ...
      const response = await axios.post<ApiResponse<Blogs>>('http://localhost:3030/v1/blogs/', formData, {
        headers: {
  ...
  const updateBlog = async (id: string, data: UpdateBlog) => {
    try {
      const response = await axios.put<ApiResponse<Blogs>>(`http://localhost:3030/v1/blogs/${id}`, data);
  ...
  const deleteBlog = async (id: string) => {
    try {
      const response = await axios.delete<ApiResponse<null>>(`http://localhost:3030/v1/blogs/${id}`);
