...
export const useBlogStore = defineStore('blogs', () => {
  ...
  const createBlog = async (data: BlogFormState) => {
    try {
      ...
      const response = await axios.post<ApiResponse<Blogs>>('https://root-foundation-school.vercel.app/v1/blogs/', formData, {
        headers: {
  ...
  const updateBlog = async (id: string, data: UpdateBlog) => {
    try {
      const response = await axios.put<ApiResponse<Blogs>>(`https://root-foundation-school.vercel.app/v1/blogs/${id}`, data);
  ...
  const deleteBlog = async (id: string) => {
    try {
      const response = await axios.delete<ApiResponse<null>>(`https://root-foundation-school.vercel.app/v1/blogs/${id}`);
